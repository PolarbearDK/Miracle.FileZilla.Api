...
if($pack) {
	& msbuild Miracle.FileZilla.Api/Miracle.FileZilla.Api.csproj /p:Configuration=release
	if(!$?){throw "msbuild returned exit code $LASTEXITCODE"}
	
	& .NuGet\NuGet.exe pack Miracle.FileZilla.Api/Miracle.FileZilla.Api.csproj -prop Configuration=release
